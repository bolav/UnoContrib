...
{
    internal enum ContactFeatureType
    {
        Vertex = 0,
        Face = 1,
    }
    ...
    {
        public float2 LocalPoint;		///< usage depends on manifold type
    ...
    {
	    public ManifoldPoint[] _points = new ManifoldPoint[2];	        ///< the points of contact
	    public float2 _localNormal;						///< not use for Type.SeparationFunction.Points
	    public float2 _localPoint;							///< usage depends on manifold type
    ...
    public struct WorldManifold
    {
        ...
        {
            _points = new float2[2];

	        if (manifold._pointCount == 0)
	        {
                _normal = float2(0.0f, 1.0f);
		        return;
	        }
	        ...
	        case ManifoldType.Circles:
		        {
			        float2 pointA = MathUtils.Multiply(ref xfA, manifold._localPoint);
                    float2 pointB = MathUtils.Multiply(ref xfB, manifold._points[0].LocalPoint);
                    ...
                    float2 cA = pointA + radiusA * _normal;
                    float2 cB = pointB - radiusB * _normal;
	        ...
	        case ManifoldType.FaceA:
		        {
			        ...
			        {
				        float2 clipPoint = MathUtils.Multiply(ref xfB, manifold._points[i].LocalPoint);
                        float2 cA = clipPoint + (radiusA - Uno.Vector.Dot(clipPoint - planePoint, _normal)) * _normal;
                        float2 cB = clipPoint - radiusB * _normal;
	        ...
	        case ManifoldType.FaceB:
		        {
			        ...
			        for (int i = 0; i < manifold._pointCount; ++i)
			        {
                        float2 clipPoint = MathUtils.Multiply(ref xfA, manifold._points[i].LocalPoint);
                        float2 cA = clipPoint - radiusA * _normal;
                        float2 cB = clipPoint + (radiusB - Uno.Vector.Dot(clipPoint - planePoint, _normal)) * _normal;
				        _points[i] = 0.5f * (cA + cB);
			        }
	    ...
	    public float2 _normal;						///< world vector pointing from A to B
	    public float2[] _points;   	///< world contact point (point of intersection)
    }
    ...
    public struct AABB
    {
	    ...
	    {
		    lowerBound = MathUtils.Min(lowerBound, aabb.lowerBound);
		    upperBound = MathUtils.Max(upperBound, aabb.upperBound);
	    }
	    ...
	    {
		    lowerBound = MathUtils.Min(aabb1.lowerBound, aabb2.lowerBound);
		    upperBound = MathUtils.Max(aabb1.upperBound, aabb2.upperBound);
	    }
	    ...
	    public bool RayCast(out RayCastOutput output, ref RayCastInput input)
        {
	        ...
	        float2 p = input.p1;
	        float2 d = input.p2 - input.p1;
	        float2 absD = MathUtils.Abs(d);
	    ...
	    public float2 lowerBound;	///< the lower vertex
    ...
    public static class Collision
    {
					          ...
					          CircleShape circleB, ref Transform xfB)
        {
	        ...
	        float2 pA = MathUtils.Multiply(ref xfA, circleA._p);
	        float2 pB = MathUtils.Multiply(ref xfB, circleB._p);

	        float2 d = pB - pA;
	        float distSqr = Uno.Vector.Dot(d, d);
	        ...
	        manifold._localNormal = float2(0);
							           ...
							           CircleShape circleB, ref Transform xfB)
        {
	        ...
	        float2 c = MathUtils.Multiply(ref xfB, circleB._p);
	        float2 cLocal = MathUtils.MultiplyT(ref xfA, c);
	        ...
	        {
                float s = Uno.Vector.Dot(polygonA._normals[i], cLocal - polygonA._vertices[i]);
            ...
            float2 v1 = polygonA._vertices[vertIndex1];
            float2 v2 = polygonA._vertices[vertIndex2];
	        ...
	        float u1 = Uno.Vector.Dot(cLocal - v1, v2 - v1);
	        float u2 = Uno.Vector.Dot(cLocal - v2, v1 - v2);
	        if (u1 <= 0.0f)
	        {
		        ...
		        manifold._localNormal = Vector.Normalize(manifold._localNormal);
	        ...
	        else
	        {
                ...
                float separation2 = Uno.Vector.Dot(cLocal - faceCenter, polygonA._normals[vertIndex1]);
                               ...
                               PolygonShape polyB, ref Transform xfB)
        {
            ...
            float2 v11 = poly1._vertices[iv1];
            float2 v12 = poly1._vertices[iv2];

	        float2 localTangent = v12 - v11;
            localTangent = Vector.Normalize(localTangent);
            ...
            // Face offset.
	        float frontOffset = Uno.Vector.Dot(normal, v11);

            // Side offsets, extended by polytope skin thickness.
            float sideOffset1 = -Uno.Vector.Dot(tangent, v11) + totalRadius;
            float sideOffset2 = Uno.Vector.Dot(tangent, v12) + totalRadius;
	        ...
	        {
		        float separation = Uno.Vector.Dot(normal, clipPoints2[i].v) - frontOffset;
					           ...
					           CircleShape circleB, ref Transform xfB)
        {
	        ...
	        float2 Q = MathUtils.MultiplyT(ref xfA, MathUtils.Multiply(ref xfB, circleB._p));

	        float2 A = edgeA._vertex1, B = edgeA._vertex2;
	        float2 e = B - A;

	        // Barycentric coordinates
	        float u = Uno.Vector.Dot(e, B - Q);
	        float v = Uno.Vector.Dot(e, Q - A);
	        ...
	        if (v <= 0.0f)
	        {
		        ...
		        {
			        float2 A1 = edgeA._vertex0;
			        float2 B1 = A;
			        float2 e1 = B1 - A1;
			        float u1 = Uno.Vector.Dot(e1, B1 - Q);
	        ...
	        if (u <= 0.0f)
	        {
		        ...
		        {
			        float2 B2 = edgeA._vertex3;
			        float2 A2 = B;
			        float2 e2 = B2 - A2;
			        float v2 = Uno.Vector.Dot(e2, Q - A2);
	        ...
	        // Region AB
	        float den = Uno.Vector.Dot(e, e);
	        P = (1.0f / den) * (u * A + v * B);
	        d = Q - P;
	        float dd2 = Uno.Vector.Dot(d, d);
        ...
        static EPAxis ComputeEdgeSeperation(float2 v1, float2 v2, float2 n, PolygonShape polygonB, float radius)
        {
	        // EdgeA separation
	        EPAxis axis;
	        axis.type = EPAxisType.EdgeA;
	        axis.index = 0;
	        axis.separation = Uno.Vector.Dot(n, polygonB._vertices[0] - v1);
	        for (int i = 1; i < polygonB._vertexCount; ++i)
	        {
		        float s = Uno.Vector.Dot(n, polygonB._vertices[i] - v1);
        ...
        static EPAxis ComputePolygonSeperation(float2 v1, float2 v2, float2 n, PolygonShape polygonB, float radius)
        {
	        // PolygonB separation
	        EPAxis axis;
	        ...
	        {
		        float s1 = Uno.Vector.Dot(polygonB._normals[i], v1 - polygonB._vertices[i]);
		        float s2 = Uno.Vector.Dot(polygonB._normals[i], v2 - polygonB._vertices[i]);
        ...
        static void FindIncidentEdge(ref ClipVertex[] c, PolygonShape poly1, int edge1, PolygonShape poly2)
        {
	        int count1 = poly1._vertexCount;
	        int count2 = poly2._vertexCount;

	        // Get the normal of the reference edge in poly2's frame.
            float2 normal1 = poly1._normals[edge1];
	        ...
	        {
                float dot = Uno.Vector.Dot(normal1, poly2._normals[i]);
		        if (dot < minDot)
		        {
			        minDot = dot;
			        index = i;
					           ...
					           PolygonShape polygonB_in, ref Transform xfB)
        {
	        ...
	        // Edge geometry
	        float2 v1 = edgeA._vertex1;
	        float2 v2 = edgeA._vertex2;
	        float2 e = v2 - v1;
	        float2 edgeNormal = float2(e.Y, -e.X);
            edgeNormal = Vector.Normalize(edgeNormal);

	        // Determine side
            bool isFrontSide = Uno.Vector.Dot(edgeNormal, s_polygonB._centroid - v1) >= 0.0f;
            ...
            EdgeType[] types = new EdgeType[2];
            //types[0] = EdgeType.Isolated;
            //types[1] = EdgeType.Isolated;
	        if (edgeA._hasVertex0)
	        {
		        float2 v0 = edgeA._vertex0;
		        float s = Uno.Vector.Dot(edgeNormal, v0 - v1);
	        ...
	        if (edgeA._hasVertex3)
	        {
		        float2 v3 = edgeA._vertex3;
		        float s = Uno.Vector.Dot(edgeNormal, v3 - v2);
		        if (s > 0.1f * Settings.b2_linearSlop)
		        {
	        ...
	        for (int i = 0; i < Settings.b2_maxManifoldPoints; ++i)
	        {
		        ...
		        separation = Uno.Vector.Dot(normal, clipPoints2[i].v) - frontOffset;
        ...
        public static int ClipSegmentToLine(out ClipVertex[] vOut, ref ClipVertex[] vIn,
							        float2 normal, float offset, int vertexIndexA)
        {
	        ...
	        float distance0 = Uno.Vector.Dot(normal, vIn[0].v) - offset;
	        float distance1 = Uno.Vector.Dot(normal, vIn[1].v) - offset;
							        ...
							        PolygonShape poly2, ref Transform xf2)
        {
	        ...
	        for (int i = 0; i < count2; ++i)
	        {
		        float dot = Uno.Vector.Dot(poly2._vertices[i], normal1);
		        if (dot < minDot)
		        {
			        minDot = dot;
			        index = i;
	        ...
	        float2 v1 = MathUtils.Multiply(ref xf1, poly1._vertices[edge1]);
	        float2 v2 = MathUtils.Multiply(ref xf2, poly2._vertices[index]);

            float separation = Uno.Vector.Dot(v2 - v1, normal1World);
            return separation;
        }
								        ...
								        PolygonShape poly2, ref Transform xf2)
        {
	        ...
	        {
		        float dot = Uno.Vector.Dot(poly1._normals[i], dLocal1);
		        if (dot > maxDot)
		        {
			        maxDot = dot;
			        edge = i;
        ...
        static void FindIncidentEdge(out ClipVertex[] c,
