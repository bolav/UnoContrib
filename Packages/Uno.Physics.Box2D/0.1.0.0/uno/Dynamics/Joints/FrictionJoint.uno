...
namespace Uno.Physics.Box2D
{
    ...
    public class FrictionJoint : Joint
    {
        ...
        {
	        float2 F = (inv_dt * _linearImpulse);
	        return F;
        }
        ...
        internal override void InitVelocityConstraints(ref TimeStep step)
        {
	        ...
	        if (step.warmStarting)
	        {
		        ...
		        float2 P = float2(_linearImpulse.X, _linearImpulse.Y);
        ...
        internal override void SolveVelocityConstraints(ref TimeStep step)
        {
            ...
            {
                float2 Cdot = vB + MathUtils.Cross(wB, rB) - vA - MathUtils.Cross(wA, rA);

                float2 impulse = -MathUtils.Multiply(ref _linearMass, Cdot);
                float2 oldImpulse = _linearImpulse;
                _linearImpulse += impulse;

                float maxImpulse = step.dt * _maxForce;

                if (Vector.LengthSquared(_linearImpulse) > maxImpulse * maxImpulse)
                {
                    _linearImpulse = Vector.Normalize(_linearImpulse);
	    ...
	    internal float2 _linearImpulse;
