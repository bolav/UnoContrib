    ...
    public class LineJoint : Joint
    {
	    ...
	    internal override void InitVelocityConstraints(ref TimeStep step)
        {
	        ...
	        if (step.warmStarting)
	        {
		        ...
		        float2 P = _impulse.X * _perp + (_motorImpulse + _impulse.Y) * _axis;
	    ...
	    internal override void SolveVelocityConstraints(ref TimeStep step)
        {
	        ...
	        float Cdot1 = Uno.Vector.Dot(_perp, v2 - v1) + _s2 * w2 - _s1 * w1;

	        if (_enableLimit && _limitState != LimitState.Inactive)
	        {
		        // Solve prismatic and limit constraint in block form.
		        float Cdot2 = Uno.Vector.Dot(_axis, v2 - v1) + _a2 * w2 - _a1 * w1;
		        float2 Cdot = float2(Cdot1, Cdot2);

		        float2 f1 = _impulse;
		        float2 df =  _K.Solve(-Cdot);
		        _impulse += df;
		        ...
		        float2 P = df.X * _perp + df.Y * _axis;
	    ...
	    internal override bool SolvePositionConstraints(float baumgarte)
        {
	        ...
	        float2 impulse = float2(0);
	        float C1;
	        C1 = Uno.Vector.Dot(_perp, d);
	        ...
	        float2 P = impulse.X * _perp + impulse.Y * _axis;
	    ...
	    internal float2 _localXAxis1;
	    internal float2 _localYAxis1;

	    internal float2 _axis, _perp;
	    internal float _s1, _s2;
	    internal float _a1, _a2;

	    internal Mat22 _K;
	    internal float2 _impulse;
