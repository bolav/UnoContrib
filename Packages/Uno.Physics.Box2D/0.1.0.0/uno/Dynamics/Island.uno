...
namespace Uno.Physics.Box2D
{
    ...
    class Island
    {
	    ...
	    public void Solve(ref TimeStep step, float2 gravity, bool allowSleep)
        {
	        ...
	        for (int i2 = 0; i2 < _contactCount; ++i2)
	        {
                ...
                bool nonStatic = bodyA.GetBodyType() != BodyType.Static && bodyB.GetBodyType() != BodyType.Static;
		        if (nonStatic)
		        {
			        ++i1;
	        ...
	        for (int i = 0; i < _bodyCount; ++i)
	        {
		        ...
		        float2 translation = step.dt * b._linearVelocity;
		        if (Uno.Vector.Dot(translation, translation) > Settings.b2_maxTranslationSquared)
		        {
			        float ratio = Settings.b2_maxTranslation / Vector.Length(translation);
	        ...
	        if (allowSleep)
	        {
		        ...
		        for (int i = 0; i < _bodyCount; ++i)
		        {
			        ...
			        if ((b._flags & BodyFlags.AutoSleep) == 0 ||
				        ...
				        Uno.Vector.Dot(b._linearVelocity, b._linearVelocity) > linTolSqr)
			        {
				        b._sleepTime = 0.0f;
				        minSleepTime = 0.0f;
			        }
	    ...
	    public void Report(ContactConstraint[] constraints)
        {
            ...
            for (int i = 0; i < _contactCount; ++i)
	        {
				...
				if(cc == null) cc = constraints[i] = new ContactConstraint();
