...
namespace Uno.Physics.Box2D
{
    ...
    public class World
    {
	    ...
	    public IContactListener ContactListener
        {
            ...
            set
            {
				debug_log "Set contact listener";
		...
		public static World Current
		{
			get
			{
				if(world == null)
				{
					world = new World(float2(0, -10.0f), false);
				}
				return world;
			}
			set
	    ...
	    public Joint CreateJoint(JointDef def)
        {
	        ...
	        bool staticA = bodyA.GetBodyType() == BodyType.Static;
	        bool staticB = bodyB.GetBodyType() == BodyType.Static;
	    ...
	    public void DrawDebugData()
        {
	        ...
	        if ((flags & DebugDrawFlags.Shape) == DebugDrawFlags.Shape)
	        {
		        for (Body b = _bodyList; b != null; b = b.GetNext())
		        {
			        ...
			        for (Fixture f = b.GetFixtureList(); f != null; f = f.GetNext())
			        {
                        ...
                        {
							MassData data;
							f.GetMassData(out data);
                            DrawShape(f, xf, float4(0.5f, 0.5f, 0.3f, 1.0f), data.center);
                        }
                        else if (b.GetBodyType() == BodyType.Static)
                        {
							MassData data;
							f.GetMassData(out data);
                            DrawShape(f, xf, float4(0.5f, 0.9f, 0.5f, 1.0f), data.center);
                        }
                        else if (b.GetBodyType() == BodyType.Kinematic)
                        {
							MassData data;
							f.GetMassData(out data);
                            DrawShape(f, xf, float4(0.5f, 0.5f, 0.9f, 1.0f), data.center);
                        }
				        else if (b.IsAwake() == false)
				        {
							MassData data;
							f.GetMassData(out data);
					        DrawShape(f, xf, float4(0.6f, 0.6f, 0.6f, 1.0f), data.center);
				        }
				        else
				        {
							MassData data;
							f.GetMassData(out data);
					        DrawShape(f, xf, float4(0.9f, 0.7f, 0.7f, 1.0f), data.center);
				        }
			        }
		        }
	        }
	        ...
	        {
		        float4 color = float4(0.3f, 0.9f, 0.9f, 1.0f);
		        for (Contact c = _contactManager._contactList; c != null; c = c.GetNext())
		        {
			      /*
                   ...
                   float2 cA = aabbA.GetCenter();
                   float2 cB = aabbB.GetCenter();
	        ...
	        {
		        float4 color = float4(0.9f, 0.3f, 0.9f, 1.0f);
		        ...
		        for (Body b = _bodyList; b != null; b = b.GetNext())
		        {
			        ...
			        for (Fixture f = b.GetFixtureList(); f != null; f = f.GetNext())
			        {
                        for (int i = 0; i < f._proxyCount; ++i)
				        {
                            ...
                            var vs = new float2[8];
					        vs[0] = float2(aabb.lowerBound.X, aabb.lowerBound.Y);
					        vs[1] = float2(aabb.upperBound.X, aabb.lowerBound.Y);
					        vs[2] = float2(aabb.upperBound.X, aabb.upperBound.Y);
					        vs[3] = float2(aabb.lowerBound.X, aabb.upperBound.Y);

					        DebugDraw.DrawPolygon(ref vs, 4, color, (aabb.lowerBound+aabb.upperBound)/2);
				        }
			        }
		        }
	        }
        ...
        float RayCastCallbackWrapper(ref RayCastInput input, int proxyId)
	    {
		    ...
		    if (hit)
		    {
			    ...
			    float2 point = (1.0f - fraction) * input.p1 + fraction * input.p2;
        ...
        void SolveTOI(Body body)
        {
	        ...
	        do
	        {
		        ...
		        for (ContactEdge ce = body._contactList; ce != null; ce = ce.Next)
		        {
			        ...
			        Body other = ce.Other;
			        BodyType type = other.GetBodyType();
            ...
            if (toiOther.GetBodyType() != BodyType.Static)
	        {
        ...
        void SolveTOI()
        {
	        ...
	        for (Body body = _bodyList; body != null; body = body._next)
	        {
		        ...
		        // If a body was not in an island then it did not move.
		        if ((body._flags & BodyFlags.Island) == 0 || body.GetBodyType() == BodyType.Kinematic || body.GetBodyType() == BodyType.Static)
		        {
	    ...
	    void DrawJoint(Joint joint)
        {
	        ...
	        float4 color = float4(0.5f, 0.8f, 0.8f, 1.0f);

	        switch (joint.JointType)
	        {
	        case JointType.Distance:
		        DebugDraw.DrawSegment(p1, p2, color, (p1+p2)/2);
		        break;
	        ...
	        case JointType.Pulley:
		        {
			        ...
			        DebugDraw.DrawSegment(s1, p1, color, (s1+p1)/2);
			        DebugDraw.DrawSegment(s2, p2, color, (s2+p2)/2);
			        DebugDraw.DrawSegment(s1, s2, color, (s1+s2)/2);
		        }
		        break;
	        ...
	        default:
		        DebugDraw.DrawSegment(x1, p1, color, (x1+p1)/2);
		        DebugDraw.DrawSegment(p1, p2, color, (p1+p2)/2);
		        DebugDraw.DrawSegment(x2, p2, color, (x2+p2)/2);
                break;
	        }
        }
	        ...
	        case ShapeType.Polygon:
		        {
			        ...
			        DebugDraw.DrawSolidPolygon(ref vertices, vertexCount, color, center);
		        }
		        break;
            ...
            case ShapeType.Edge:
		        {
			        ...
			        float2 v1 = MathUtils.Multiply(ref xf, edge._vertex1);
			        float2 v2 = MathUtils.Multiply(ref xf, edge._vertex2);
	        ...
	        case ShapeType.Loop:
		        {
			        ...
			        float2 v1 = MathUtils.Multiply(ref xf, loop._vertices[count - 1]);
			        for (int i = 0; i < count; ++i)
			        {
				        float2 v2 = MathUtils.Multiply(ref xf, loop._vertices[i]);
				        DebugDraw.DrawSegment(v1, v2, color, center);
				        v1 = v2;
			        }
		        }
