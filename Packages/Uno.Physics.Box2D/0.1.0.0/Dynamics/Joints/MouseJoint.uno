...
public class MouseJoint : Joint
{
	...
	internal override void InitVelocityConstraints(ref TimeStep step)
    {
        ...
        var K1 = new Mat22(float2(invMass, 0.0f), float2(0.0f, invMass));
        var K2 = new Mat22(float2(invI * r.Y * r.Y, -invI * r.X * r.Y), float2(-invI * r.X * r.Y, invI * r.X * r.X));
	...
	internal override void SolveVelocityConstraints(ref TimeStep step)
    {
	    ...
	    float2 impulse = MathUtils.Multiply(ref _mass, -(Cdot + _beta * _C + _gamma * _impulse));

	    float2 oldImpulse = _impulse;
	    _impulse += impulse;
	    float maxImpulse = step.dt * _maxForce;
        if (Vector.LengthSquared(_impulse) > maxImpulse * maxImpulse)
	    {
		    _impulse *= maxImpulse / Vector.Length(_impulse);
	    }
	...
	public float2 _localAnchor;
    public float2 _target;
    public float2 _impulse;

    public Mat22 _mass;		// effective mass for point-to-point constraint.
    public float2 _C;				// position error
