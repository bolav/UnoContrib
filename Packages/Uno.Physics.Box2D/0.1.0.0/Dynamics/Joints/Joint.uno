...
namespace Uno.Physics.Box2D
{
    ...
    {
	    public float2 linearA;
	    public float angularA;
	    public float2 linearB;
	    public float angularB;

	    public void SetZero()
        {
	        linearA = float2(0); angularA = 0.0f;
	        linearB = float2(0); angularB = 0.0f;
        }

	    public void Set(float2 x1, float a1, float2 x2, float a2)
        {
	        linearA = x1; angularA = a1;
	        linearB = x2; angularB = a2;
        }

	    public float Compute(float2 x1, float a1, float2 x2, float a2)
        {
            return Uno.Vector.Dot(linearA, x1) + angularA * a1 + Uno.Vector.Dot(linearB, x2) + angularB * a2;
        }
    }

     /// A joint edge is used to connect bodies and joints together
    /// in a joint graph where each body is a box2dNode and each joint
    ...
    public abstract class Joint
    {
	    ...
	    public abstract float2 GetAnchorA();

	    /// Get the anchor point on body2 in world coordinates.
	    public abstract float2 GetAnchorB();

        /// Get the reaction force on body2 at the joint anchor in Newtons.
	    public abstract float2 GetReactionForce(float inv_dt);
	    ...
	    internal float2 _localCenterA, _localCenterB;
