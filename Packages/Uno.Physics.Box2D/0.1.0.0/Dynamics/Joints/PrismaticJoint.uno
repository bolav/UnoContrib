...
namespace Uno.Physics.Box2D
{
    ...
    public class PrismaticJoint : Joint
    {
	    ...
	    public float GetJointTranslation()
        {
	        ...
	        float2 p1 = b1.GetWorldPoint(_localAnchor1);
	        float2 p2 = b2.GetWorldPoint(_localAnchor2);
	        float2 d = p2 - p1;
	        float2 axis = b1.GetWorldVector(_localXAxis1);

	        float translation = Uno.Vector.Dot(d, axis);
	        return translation;
        }
	    ...
	    public float GetJointSpeed()
        {
	        ...
	        float2 v1 = b1._linearVelocity;
	        float2 v2 = b2._linearVelocity;
	        float w1 = b1._angularVelocity;
	        float w2 = b2._angularVelocity;

	        float speed = Uno.Vector.Dot(d, MathUtils.Cross(w1, axis)) + Uno.Vector.Dot(axis, v2 + MathUtils.Cross(w2, r2) - v1 - MathUtils.Cross(w1, r1));
	        return speed;
        }
	    ...
	    internal override void InitVelocityConstraints(ref TimeStep step)
        {
	        ...
	        // Prismatic constraint.
	        {
		        ...
		        _K.col1 = float3(k11, k12, k13);
		        _K.col2 = float3(k12, k22, k23);
		        _K.col3 = float3(k13, k23, k33);
	        }

	        // Compute motor and limit terms.
	        if (_enableLimit)
	        {
		        float jointTranslation = Uno.Vector.Dot(_axis, d);
	        ...
	        if (step.warmStarting)
	        {
		        ...
		        float2 P = _impulse.X * _perp + (_motorImpulse + _impulse.Z) * _axis;
	    ...
	    internal override void SolveVelocityConstraints(ref TimeStep step)
        {
	        ...
	        {
		        float Cdot = Uno.Vector.Dot(_axis, v2 - v1) + _a2 * w2 - _a1 * w1;
            ...
            float2 Cdot1 = float2(Uno.Vector.Dot(_perp, v2 - v1) + _s2 * w2 - _s1 * w1, w2 - w1);

	        if (_enableLimit && _limitState != LimitState.Inactive)
	        {
		        // Solve prismatic and limit constraint in block form.
		        float Cdot2 = Uno.Vector.Dot(_axis, v2 - v1) + _a2 * w2 - _a1 * w1;
		        float3 Cdot = float3(Cdot1.X, Cdot1.Y, Cdot2);

		        float3 f1 = _impulse;
		        float3 df =  _K.Solve33(-Cdot);
		        _impulse += df;
		        ...
		        // f2(1:2) = invK(1:2,1:2) * (-Cdot(1:2) - K(1:2,3) * (f2(3) - f1(3))) + f1(1:2)
		        float2 b = -Cdot1 - (_impulse.Z - f1.Z) * float2(_K.col3.X, _K.col3.Y);
		        float2 f2r = _K.Solve22(b) + float2(f1.X, f1.Y);
		        _impulse.X = f2r.X;
		        _impulse.Y = f2r.Y;

		        df = _impulse - f1;

		        float2 P = df.X * _perp + df.Z * _axis;
	    ...
	    internal override bool SolvePositionConstraints(float baumgarte)
        {
	        ...
	        float2 c1 = b1._sweep.c;
	        float a1 = b1._sweep.a;

	        float2 c2 = b2._sweep.c;
	        ...
	        float2 r1 = MathUtils.Multiply(ref R1, _localAnchor1 - _localCenterA);
	        float2 r2 = MathUtils.Multiply(ref R2, _localAnchor2 - _localCenterB);
	        float2 d = c2 + r2 - c1 - r1;
	        ...
	        if (_enableLimit)
	        {
		        ...
		        float translation = Uno.Vector.Dot(_axis, d);
		        if (Math.Abs(_upperTranslation - _lowerTranslation) < 2.0f * Settings.b2_linearSlop)
		        {
            ...
            float2 C1 = float2(Uno.Vector.Dot(_perp, d), a2 - a1 - _refAngle);
	        ...
	        else
	        {
		        ...
		        _K.col1 = float3(k11, k12, 0.0f);
		        _K.col2 = float3(k12, k22, 0.0f);

		        float2 impulse1 = _K.Solve22(-C1);
		        impulse.X = impulse1.X;
		        impulse.Y = impulse1.Y;
		        impulse.Z = 0.0f;
	        }

	        float2 P = impulse.X * _perp + impulse.Z * _axis;
	    ...
	    public float2 _localAnchor1;
        public float2 _localAnchor2;
        public float2 _localXAxis1;
        public float2 _localYAxis1;
        public float _refAngle;

        public float2 _axis, _perp;
        public float _s1, _s2;
        public float _a1, _a2;

        public Mat33 _K;
        public float3 _impulse;
