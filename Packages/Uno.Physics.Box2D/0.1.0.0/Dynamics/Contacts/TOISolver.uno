...
{
    internal class TOIConstraint
    {
	    public float2[] localPoints = new float2[2];
		    ...
		    case ManifoldType.Circles:
			    {
				    ...
				    float2 pointB = cc.bodyB.GetWorldPoint(cc.localPoints[0]);
                    if (Vector.LengthSquared(pointA - pointB) > Settings.b2_epsilon * Settings.b2_epsilon)
				    {
				    ...
				    separation = Uno.Vector.Dot(pointB - pointA, normal) - cc.radius;
			    }
			    break;
		    ...
		    case ManifoldType.FaceA:
			    {
				    ...
				    float2 clipPoint = cc.bodyB.GetWorldPoint(cc.localPoints[index]);
				    separation = Uno.Vector.Dot(clipPoint - planePoint, normal) - cc.radius;
				    point = clipPoint;
			    }
			    break;
		    ...
		    case ManifoldType.FaceB:
			    {
				    ...
				    float2 clipPoint = cc.bodyA.GetWorldPoint(cc.localPoints[index]);
				    separation = Uno.Vector.Dot(clipPoint - planePoint, normal) - cc.radius;
				    point = clipPoint;
    ...
    internal class TOISolver
    {
        ...
        public void Initialize(Contact[] contacts, int count, Body toiBody)
        {
            ...
            for (int i = 0; i < _count; ++i)
            {
				...
				if(_constraints[i] == null) _constraints[i] = new TOIConstraint();
        ...
        public bool Solve(float baumgarte)
        {
            ...
            for (int i = 0; i < _count; ++i)
            {
                ...
                {
                    var psm = new TOISolverManifold(ref c, j);
