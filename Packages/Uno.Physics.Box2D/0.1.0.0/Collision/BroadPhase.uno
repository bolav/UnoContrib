...
namespace Uno.Physics.Box2D
{
    ...
    public class BroadPhase
    {
	    ...
	    public void MoveProxy(int proxyId, ref AABB aabb, float2 displacement)
        {
	    ...
	    public void UpdatePairs<T>(Uno.Action<T,T> callback)
        {
            // Reset pair buffer
	        _pairCount = 0;
        ...
        private int OnComparePair(Pair pair, Pair pair1)
        {
            return pair.CompareTo(pair1);
        }

        /// Query an AABB for overlapping proxies. The callback class
	    /// is called for each proxy that overlaps the supplied AABB.
	    public void Query(Uno.Func<int, bool> callback, ref AABB aabb)
        {
